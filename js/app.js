...
function reminder(string,link,num,ary){
    ...
    if (getBt(string)>32){
       ...
       str+='<div class="text-center line_height25" id='+b+'>';
        str+='<div class="sign_foot"  style="word-wrap: break-word">'+string+'</div>';
    }
    ...
    if(query('#' + d)) {
        query('#' + d).onclick = function () {
            if(Object.prototype.toString.call(link)==="[object Function]"){
                link();
            }else if (link) {
...
(function(){
    if(queryAll('select')){
        var reg=/请选择|请输入|请/g;
        var sel=queryAll('select');
        for(var i= 0,len=sel.length;i<len;i++){
            var that=sel[i]
            setCss(that,'font-family',"inherit");
            if(reg.test(that.value)){
                addClass(that,'colorG5');
                removeClass(that,'colorG1');
            } else {
                addClass(that,'colorG1');
                removeClass(that,'colorG5');
                setCss(that,{'font-family':''})
            }
            that.onchange=function(){
                if(reg.test(this.value)){
                    addClass(this,'colorG5');
                    removeClass(this,'colorG1');
                } else {
                    addClass(this,'colorG1');
                    removeClass(this,'colorG5');

                }
            }
        }
    }
    if(queryAll('textarea')){
        window.onload=function(){
            var tex=queryAll('textarea');
            for(var j= 0,lon=tex.length;j<lon;j++){
                var those=tex[j];
                var scoHeight=those.scrollHeight;
                ~function(){
                    var padTop= getCss(those,'padding-top');
                    scoHeight=scoHeight-padTop+'px';
                    setCss(those,'height',scoHeight);
                    those.onchange=function(){
                        var scoHeight=this.scrollHeight-padTop+'px';
                        setCss(this,'height',scoHeight)
                    }
                }()
            }
        }
